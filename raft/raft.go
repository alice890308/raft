...
func (r *Raft) appendEntries(req *pb.AppendEntriesRequest) (*pb.AppendEntriesResponse, error) {
	...
	incomingTerm := req.GetTerm()
	if incomingTerm < r.currentTerm {
		r.logger.Info("reject append entries since term in current request is older")
	...
	if r.state != Follower && r.id != req.GetLeaderId() {
		r.toFollower(incomingTerm)
...
func (r *Raft) requestVote(req *pb.RequestVoteRequest) (*pb.RequestVoteResponse, error) {
	...
	if incomingTerm < r.currentTerm {
		r.logger.Info("reject request vote since term in current request is older")
	...
	curLogID, curLogTerm := r.getLastLog()
	if curLogTerm > req.GetLastLogTerm() || curLogID > req.GetLastLogId() {
		// TODO: (A.7) - if votedFor is null or candidateId, and candidate’s log is at least as up-to-date as receiver’s log, grant vote
